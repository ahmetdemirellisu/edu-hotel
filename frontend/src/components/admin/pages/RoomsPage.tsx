...
const LAYOUT: Cell[] = [
  [234, 3, 0, 2, 2], [232, 5, 0, 2, 2], [230, 7, 0, 2, 2],
  ...
  [238, 0, 8, 2, 2], [240, 0, 10, 2, 2], [242, 0, 12, 2, 3],
  [244, 0, 15, 2, 2], [246, 0, 17, 2, 2], [248, 0, 19, 2, 2], [250, 0, 21, 2, 2],
  [227, 3, 6, 2, 2], [239, 3, 9, 2, 2], [241, 3, 11, 2, 2],
  [243, 3, 13, 2, 2], [245, 3, 15, 2, 2], [247, 3, 17, 2, 2],
  [249, 3, 19, 2, 2], [251, 3, 21, 2, 2],
  [217, 22, 5, 2, 2], [216, 22, 7, 2, 2],
  [215, 22, 9, 2, 2], [213, 22, 11, 2, 2], [212, 22, 13, 2, 2],
  [211, 22, 15, 2, 2], [209, 22, 17, 2, 2], [207, 22, 19, 2, 2],
  [205, 22, 21, 2, 2], [203, 22, 23, 2, 2], [201, 22, 25, 2, 2],
  [214, 25, 9, 2, 2], [210, 25, 11, 2, 3],
  [208, 25, 14, 2, 2], [206, 25, 16, 2, 2], [204, 25, 18, 2, 2],
];
...
export function RoomsPage() {
  ...
  const FloorPlan = () => (
    <div className="flex gap-0">
      <div className="flex-1 bg-white rounded-2xl border border-gray-200 shadow-sm p-5 overflow-auto">
        <svg viewBox={`-10 -10 ${SVG_W + 20} ${SVG_H + 20}`} className="w-full mx-auto block" style={{ maxWidth: 800 }}>
          {/* Building outline */}
          <rect x={-4} y={-4} width={SVG_W + 8} height={SVG_H + 8} rx={14} fill="none" stroke="#e2e8f0" strokeWidth={1.5} strokeDasharray="6 4" />

          {/* Corridors */}
          <rect x={2.5 * C} y={2.2 * C} width={21 * C} height={C * 0.6} rx={4} fill="#f1f5f9" />
          <rect x={2.2 * C} y={2.5 * C} width={C * 0.6} height={20.5 * C} rx={4} fill="#f1f5f9" />
          <rect x={24 * C} y={4 * C} width={C * 0.6} height={22 * C} rx={4} fill="#f1f5f9" />
          <text x={13 * C} y={2.65 * C} textAnchor="middle" fontSize={7} fill="#94a3b8" letterSpacing={5} fontWeight={700}>KORİDOR</text>

          {/* Service area */}
          <rect x={9.5 * C} y={0} width={4 * C} height={2 * C} rx={6} fill="#f8fafc" stroke="#e2e8f0" strokeWidth={0.8} />
          <text x={11.5 * C} y={C + 2} textAnchor="middle" fontSize={7} fill="#94a3b8" fontWeight={600}>2055</text>

          {/* Courtyard */}
          <rect x={6 * C} y={7 * C} width={15 * C} height={14 * C} rx={10}
            fill="#fafbff" stroke="#e2e8f0" strokeWidth={1} />
          <text x={13.5 * C} y={13.5 * C} textAnchor="middle" fontSize={11} fill="#94a3b8" fontWeight={700} letterSpacing={5}>AVLU</text>
          <text x={13.5 * C} y={15 * C} textAnchor="middle" fontSize={8} fill="#cbd5e1" letterSpacing={3}>COURTYARD</text>
          ...
          {[
            ...
            <text key={i} x={x as number} y={y as number} fontSize={6} fill="#d1d5db" fontWeight={600} letterSpacing={2}>{text as string}</text>
          ))}
          ...
          {LAYOUT.map(([num, col, row, w, h]) => {
            ...
            return (
              <g key={num} style={{ cursor: "pointer" }}
                ...
                {act && <rect x={x + 2} y={y + 3} width={rw} height={rh} rx={6} fill="rgba(0,0,0,0.08)" />}
                {/* Room rect */}
                <rect x={x + 1} y={y + 1} width={rw} height={rh} rx={6}
                  fill={act ? cfg.bgH : cfg.bg}
                  stroke={isS ? cfg.text : act ? cfg.dot : cfg.border}
                  strokeWidth={isS ? 2.5 : act ? 1.8 : 0.8}
                  style={{ transition: "all 0.15s ease" }} />
                {/* Status gradient accent — top edge */}
                {act && (
                  <rect x={x + 1} y={y + 1} width={rw} height={4} rx={6}
                    fill={cfg.dot} opacity={0.5} style={{ pointerEvents: "none" }} />
                )}
                {/* Room number */}
                <text x={x + 1 + rw / 2} y={y + 1 + rh / 2 + (dbl ? -3 : 1)}
                  textAnchor="middle" dominantBaseline="middle"
                  fontSize={act ? 12 : 11} fontWeight={act ? 800 : 600} fill={cfg.text}
                  style={{ pointerEvents: "none", transition: "all 0.12s" }}>
                  {num}
                </text>
                {/* Status dot */}
                <circle cx={x + rw - 4} cy={y + 7} r={3} fill={cfg.dot} opacity={act ? 1 : 0.7} />
                {/* Double label */}
                {dbl && (
                  <text x={x + 1 + rw / 2} y={y + rh - 5} textAnchor="middle"
                    fontSize={6} fill={cfg.text} fontWeight={700} opacity={0.5}
                ...
                {room.reservation && act && (
                  <foreignObject x={x - 30} y={y + rh + 3} width={rw + 60} height={32}>
                    <div style={{
                      background: "rgba(15,23,42,0.92)", color: "#fff", fontSize: 9,
                      padding: "4px 9px", borderRadius: 7, textAlign: "center",
                      whiteSpace: "nowrap", fontWeight: 600, backdropFilter: "blur(6px)",
          ...
          {/* Floor label */}
          <text x={SVG_W / 2} y={SVG_H + 8} textAnchor="middle" fontSize={8} fill="#94a3b8" fontWeight={700} letterSpacing={4}>
      ...
      {rooms.map(room => {
        ...
        return (
          <div key={room.id} onClick={() => setSelected(room)}
            className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all duration-150 cursor-pointer overflow-hidden group hover:-translate-y-0.5">
            {/* Gradient bar */}
            <div className="h-1.5 rounded-t-2xl" style={{ background: `linear-gradient(90deg, ${cfg.gradientFrom}, ${cfg.gradientTo})` }} />
            <div className="p-4">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-sm font-bold text-gray-900">Room {room.name}</h4>
              ...
              <div className="text-[11px] text-gray-500 space-y-0.5">
                ...
                <p className="text-gray-400">₺{room.price}{t("rooms.grid.perNight", "/night")}</p>
              </div>
              {room.reservation && (
                <div className="mt-2 bg-red-50 border border-red-100 rounded-xl p-2 text-[10px]">
                  <p className="font-bold text-red-700 truncate">{room.reservation.guestName}</p>
                  <p className="text-red-400 font-mono">#{room.reservation.id}</p>
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <div className="space-y-5">
      <style>{`
        @keyframes roomIn { from { opacity:0; transform: translateY(8px); } to { opacity:1; transform: translateY(0); } }
      `}</style>
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">{t("pages.rooms.title", "Room Management")}</h2>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 h-9 shadow-sm">
              <Calendar className="h-3.5 w-3.5 text-gray-400 flex-shrink-0" />
              <input type="date" value={date} onChange={e => setDate(e.target.value)}
                className="bg-transparent text-sm font-semibold text-gray-800 outline-none w-[120px]" />
            </div>
            <button
              onClick={() => goDay(1)}
              className="w-9 h-9 rounded-xl bg-gray-50 border border-gray-200 hover:bg-gray-100 flex items-center justify-center transition-colors shadow-sm"
            ...
            {!isToday && (
              <button
                ...
                className="px-3 h-9 rounded-xl bg-gradient-to-r from-[#003366] to-[#0055aa] text-white text-xs font-bold shadow-md hover:shadow-lg transition-all duration-150"
              >
                {t("rooms.today", "Today")}
              </button>
            )}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {(["AVAILABLE", "OCCUPIED", "MAINTENANCE", "RESERVED"] as StatusKey[]).map(s => {
              ...
              return (
                <div
                  key={s}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-[11px] font-bold shadow-sm ${cfg.pillBg} ${cfg.pillText} ${cfg.pillBorder}`}
                >
                  <span className="w-2 h-2 rounded-full" style={{ background: cfg.dot }} />
                  {statusLabel(s)}
                  <span className="font-extrabold ml-0.5">{c}</span>
                </div>
              );
            })}
          </div>
          ...
          {/* View toggle */}
          <div className="flex bg-gray-100 rounded-xl p-0.5 shadow-sm">
      ...
      {error && (
        <div className="text-sm text-red-700 bg-red-50 border border-red-200 rounded-xl px-4 py-3 shadow-sm">{error}</div>
      )}
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center h-64 bg-white rounded-2xl border border-gray-100 shadow-sm">
          ...
          <p className="text-sm text-gray-500 font-medium">Loading floor plan…</p>
        </div>
      ) : rooms.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 bg-white rounded-2xl border border-gray-100 shadow-sm">
          <BedDouble className="h-10 w-10 mb-3 text-gray-300" />
          <p className="text-sm font-semibold text-gray-500">{t("rooms.noRooms", "No rooms found.")}</p>
        </div>
...
function Drawer({ room, date, onClose }: { room: RoomData; date: string; onClose: () => void }) {
  ...
  return (
    <div
      className="bg-white rounded-2xl border border-gray-200 shadow-xl h-full flex flex-col overflow-hidden"
      style={{ animation: "roomIn 0.2s ease-out" }}
    >
      {/* Gradient header */}
      <div className="relative overflow-hidden">
        <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${cfg.gradientFrom}22, ${cfg.gradientTo}11)` }} />
        <div className="relative flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              <span
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold border ${cfg.pillBg} ${cfg.pillText} ${cfg.pillBorder}`}
              >
                <span className="w-1.5 h-1.5 rounded-full" style={{ background: cfg.dot }} />
                {statusLabel(room.status as keyof typeof STATUS)}
              </span>
            </div>
            <h3 className="text-base font-extrabold text-gray-900">{t("rooms.roomLabel", { number: room.name, defaultValue: `Room ${room.name}` })}</h3>
      ...
      <div className="flex-1 overflow-auto px-5 py-4 space-y-4">
        ...
        <div className="bg-gray-50 rounded-xl overflow-hidden">
          {[
            ...
            { icon: <span className="text-sm font-extrabold">₺</span>, l: t("rooms.price", "Price"), v: `₺${room.price}${t("rooms.drawer.perNight", "/night")}` },
            { icon: <Wifi className="h-4 w-4" />, l: t("rooms.amenities", "Amenities"), v: room.amenities || "—" },
          ].map(({ icon, l, v }) => (
            <div key={l} className="flex items-start justify-between px-3 py-2.5 border-b border-gray-100 last:border-0">
              <div className="flex items-center gap-2 text-gray-400 text-xs font-medium">{icon}{l}</div>
              <span className="text-xs font-semibold text-gray-700 text-right max-w-[150px]">{v}</span>
            </div>
          ))}
        </div>
        ...
        {room.reservation && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-3.5">
            <p className="text-xs font-bold text-red-700 mb-1.5">{t("rooms.drawer.occupiedTitle", "Currently Occupied")}</p>
            <p className="text-sm font-semibold text-red-700">{room.reservation.guestName}</p>
        ...
        {room.status === "MAINTENANCE" && !room.reservation && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3.5">
            <p className="text-xs font-bold text-amber-700">{t("rooms.drawer.maintenanceTitle", "Under Maintenance")}</p>
        ...
        {room.status === "RESERVED" && !room.reservation && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-3.5">
            <p className="text-xs font-bold text-blue-700">{t("rooms.drawer.reservedTitle", "Reserved for Special Guests")}</p>
        ...
        <div className="pt-2 border-t border-gray-100">
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">{t("rooms.drawer.changeStatus", "Change Base Status")}</p>
          <select
            value={newStatus}
            onChange={e => setNewStatus(e.target.value)}
            className="w-full h-10 px-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-300 transition-all"
          >
          ...
          <button
            ...
            className="mt-2.5 w-full h-10 rounded-xl bg-gradient-to-r from-[#003366] to-[#0055aa] text-white text-xs font-bold shadow-md hover:shadow-lg disabled:opacity-40 disabled:shadow-none transition-all duration-150 hover:-translate-y-0.5"
          >
