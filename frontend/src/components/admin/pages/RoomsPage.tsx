...
export function RoomsPage() {
  ...
  const fmtDate = (s: string) => new Date(s + "T12:00:00").toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" });
        ...
        return (
          <div key={room.id} onClick={() => setSelected(room)}
            ...
            <div className="text-[11px] text-gray-500 space-y-0.5">
              <p>{room.type === "DOUBLE" ? "Double" : "Single"} · {room.capacity} guest{room.capacity > 1 ? "s" : ""}</p>
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">{counts.total} rooms · {fmtDate(date)}</p>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {(["AVAILABLE", "OCCUPIED", "MAINTENANCE", "RESERVED"] as StatusKey[]).map(s => {
            ...
            return (
              <div key={s} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-[11px] font-semibold"
                ...
                {cfg.label} <span className="font-bold">{c}</span>
              </div>
            );
          })}
        </div>
        ...
        <div className="flex bg-gray-100 rounded-lg p-0.5">
          {([["floor", MapIcon, "Floor Plan"], ["grid", LayoutGrid, "Cards"]] as const).map(([key, Icon, label]) => (
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-64 text-gray-400">
          ...
          <p className="text-sm">No rooms found. Run the seed script.</p>
        </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-xl h-full flex flex-col overflow-hidden"
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        <div>
          <h3 className="text-base font-bold text-gray-900">Room {room.name}</h3>
          <p className="text-[11px] text-gray-400 mt-0.5">Kat 2 · {date}</p>
        </div>
      ...
      <div className="flex-1 overflow-auto px-5 py-4 space-y-4">
        ...
        <div>
          {[
            ...
            { icon: <span className="text-sm font-bold">₺</span>, l: "Price", v: `₺${room.price}/night` },
        ...
        {room.reservation && (
          <div className="bg-red-50 border border-red-100 rounded-xl p-3.5">
            <p className="text-xs font-bold text-red-700 mb-1">Occupied</p>
            <p className="text-xs text-red-600">{room.reservation.guestName}</p>
            <p className="text-[10px] text-red-500 mt-1">
              #{room.reservation.id} · {new Date(room.reservation.checkIn).toLocaleDateString("en-GB")} → {new Date(room.reservation.checkOut).toLocaleDateString("en-GB")} · {room.reservation.guests} guest{room.reservation.guests > 1 ? "s" : ""}
            </p>
          </div>
        )}
        ...
        {room.status === "MAINTENANCE" && !room.reservation && (
          <div className="bg-amber-50 border border-amber-100 rounded-xl p-3">
            <p className="text-xs font-semibold text-amber-700">Under Maintenance</p>
            <p className="text-[10px] text-amber-600 mt-0.5">Room is unavailable for booking.</p>
          </div>
        )}

        {room.status === "RESERVED" && !room.reservation && (
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-3">
            <p className="text-xs font-semibold text-blue-700">Reserved for Special Guests</p>
            <p className="text-[10px] text-blue-600 mt-0.5">This room is permanently reserved and cannot be booked through the system.</p>
          </div>
        )}

        {/* Admin: change status */}
        <div className="pt-2 border-t border-gray-100">
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Change Base Status</p>
