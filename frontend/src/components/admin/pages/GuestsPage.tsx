...
export function GuestsPage() {
  ...
  const formatUserType = (ut: UserType) => {
    const map: Record<string, { label: string; bg: string; text: string }> = {
      STUDENT: { label: "Student", bg: "bg-sky-50", text: "text-sky-700" },
      STAFF: { label: "Staff", bg: "bg-violet-50", text: "text-violet-700" },
      SPECIAL_GUEST: { label: "Special Guest", bg: "bg-amber-50", text: "text-amber-700" },
  ...
  return (
    <div className="space-y-5">
      <div><h2 className="text-xl font-semibold text-gray-900 tracking-tight">{t("pages.guests.title", "Guests")}</h2><p className="text-sm text-gray-500 mt-0.5">{totalCount} guests · {blacklistedCount} blacklisted</p></div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select value={typeFilter} onChange={e => setTypeFilter(e.target.value as any)} className="h-9 px-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20">
            <option value="ALL">All Types</option><option value="STUDENT">Student</option><option value="STAFF">Staff</option><option value="SPECIAL_GUEST">Special Guest</option><option value="OTHER">Other</option>
          </select>
          <select value={blacklistFilter} onChange={e => setBlacklistFilter(e.target.value as any)} className="h-9 px-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20">
            <option value="all">All Statuses</option><option value="active">Active</option><option value="blacklisted">Blacklisted</option>
          </select>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center"><Users className="h-8 w-8 text-gray-300 mx-auto mb-3" /><p className="text-sm text-gray-500">No guests found.</p></div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="border-b border-gray-100 bg-gray-50/50">
                {["ID", "Name", "Email", "Type", "Reservations", "Status", "Actions"].map(h => (
                ...
                {guests.map(g => {
                  ...
                  return (
                    <tr key={g.id} className="border-b border-gray-50 last:border-b-0 hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="py-3.5 px-4 text-[13px] font-medium text-gray-800">{g.name || "Unknown"}</td>
                      ...
                      <td className="py-3.5 px-4">
                        {g.blacklist
                          ? <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-red-50 text-red-700">Blacklisted</span>
                      ...
                      <td className="py-3.5 pr-6 px-4">
                        <div className="flex items-center gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => setSelectedGuest(g)} className="w-7 h-7 rounded-lg bg-gray-50 hover:bg-gray-100 flex items-center justify-center text-gray-500 transition-colors" title="View"><Eye className="h-3.5 w-3.5" /></button>
                          {g.blacklist ? (
                            <button onClick={() => handleUnblacklist(g)} disabled={actionLoadingId === g.id} className="w-7 h-7 rounded-lg bg-emerald-50 hover:bg-emerald-100 flex items-center justify-center text-emerald-600 transition-colors disabled:opacity-40" title="Remove from blacklist"><ShieldCheck className="h-3.5 w-3.5" /></button>
                          ) : (
                            <button onClick={() => handleBlacklist(g)} disabled={actionLoadingId === g.id} className="w-7 h-7 rounded-lg bg-red-50 hover:bg-red-100 flex items-center justify-center text-red-600 transition-colors disabled:opacity-40" title="Blacklist"><ShieldX className="h-3.5 w-3.5" /></button>
                          )}
                        </div>
                      </td>
                    </tr>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={() => setSelectedGuest(null)}>
          <div className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()} style={{ animation: "adminFadeIn 0.2s ease-out" }}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div>
                <h3 className="text-lg font-semibold text-gray-900">{selectedGuest.name || "Unknown"}</h3>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              <div className="flex items-center gap-3">
                ...
                {selectedGuest.blacklist ? <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-red-50 text-red-700">Blacklisted</span> : <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700">Active</span>}
              </div>
              ...
              <div>
                <h4 className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Reservation History ({selectedGuest.reservations.length})</h4>
                {selectedGuest.reservations.length === 0 ? <p className="text-sm text-gray-400">No reservations.</p> : (
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex justify-end">
              <button onClick={() => setSelectedGuest(null)} className="px-4 py-2 rounded-xl border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">Close</button>
            </div>
          </div>
        </div>
      )}
